...
function readToken(): string {
  ...
  try {
    ...
    throw new Error(
      'FDE auth token not found. Set the FDE_BRIDGE_TOKEN environment variable ' +
      `or run native-host/install.sh (expected token at ${DEFAULT_TOKEN_PATH}).`
    );
  }
}

const FDE_RUN_TOOL: Tool = {
  name: 'fde_run',
  description:
    'Send a prompt to Palantir AI FDE (Foundry Development Environment) running in Chrome ' +
  ...
  inputSchema: {
    ...
    properties: {
      ...
      config_override: {
        type: 'object',
        description:
          'Optional FDE session configuration. Overrides ~/.config/fde-bridge/fde-config.json for this call. ' +
...
async function callFde(args: FdeArgs): Promise<string> {
  ...
  try {
    ...
    if (msg.includes('ECONNREFUSED') || msg.includes('fetch failed')) {
      throw new Error(
        'Cannot connect to FDE Bridge (http://127.0.0.1:27182). ' +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  const args = request.params.arguments as unknown as FdeArgs;

  if (!args?.prompt || typeof args.prompt !== 'string') {
    return {
      content: [{ type: 'text' as const, text: 'Error: prompt is required (string)' }],
      isError: true,
    };
  }
